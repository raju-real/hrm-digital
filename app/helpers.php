...
if (!function_exists('hasCheckIn')) {
    function hasCheckIn($employeeId)
    {
        // Get today's check-in
        $todayCheckIn = AttendanceLog::where('employee_id', $employeeId)
            ->where('type', 'manual')
...
if (!function_exists('hasCheckOut')) {
    function hasCheckOut($employeeId)
    {
        // Check if there's a check-out today
        $hasCheckOut = AttendanceLog::where('employee_id', $employeeId)
            ->where('type', 'manual')
            ->whereDate('punch_time', now()->toDateString())
            ->exists();
